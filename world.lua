...
M.rooms = {
    foyer = {
        ...
        name = "Foyer",
        x = 2, y = 2,
        exits = { "library", "study", "conservatory", "cellar" },
        description =
[[The Foyer of Ashworth Manor.
...
function M.new_state()
    return {
        ...
        unlocked       = {
            cd = true, ls = true, echo = true, help = true,
            accuse = true,
            cat = false, grep = false,
        },
...
function M.check_unlocks(state)
    -- cat unlocks once the player has stepped out of the Foyer at least once.
    for room_id, _ in pairs(state.visited) do
        if room_id ~= "foyer" then
